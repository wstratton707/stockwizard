...
OPTIMISATION = (
    "Weights are computed over up to 5 years of daily returns, from a covariance "
    "matrix estimated with Ledoit-Wolf shrinkage. Shrinkage is used because "
    "optimisation is an error-maximiser: it loads onto whichever covariances "
    "estimation noise happens to have understated.<br><br>"
    "The <b>Risk-Matched Model</b> uses no expected returns at all. Errors in "
    "expected returns damage a portfolio far more than errors in covariances, and "
    "expected returns are the harder of the two to estimate, so the risk level "
    "instead slides between three weightings that depend only on covariance: "
    "minimum-variance at 1, equal-risk-contribution (each holding supplying the same "
    "share of portfolio risk) in the middle, and equal weight at 10. Risk rises "
    "monotonically across that range. Our own walk-forward test over 10 years "
    "measured volatility rising from 17.4% to 20.6% and maximum drawdown deepening "
    "from −38.7% to −41.6% from one end to the other.<br><br>"
    "The <b>Maximum Sharpe</b> and <b>Minimum Volatility</b> models are classical "
    "mean-variance optimisation (Markowitz, 1952). Their expected returns are "
    "<b>not</b> historical averages: each holding's is CAPM (risk-free rate + beta × "
    "a 5% equity risk premium), adjusted by up to ±2%/yr for its multi-factor "
    "selection score. Because that makes expected return a function of beta, those "
    "two models tend to produce similar portfolios to each other.<br><br>"
    "Sector concentration is capped at 40%; the per-position cap is whatever you set "
    "in the Universe step (default 25%, range 5–40%). Constraints of this kind are "
    "not only realism — a no-short-sale constraint is mathematically equivalent to "
    "shrinking the covariance matrix (Jagannathan & Ma, 2003), so they improve the "
    "estimate as well as the portfolio. All of this is a mathematical output, not a "
    "personalised financial recommendation."
)
...
BUILDER_SCOPE = (
    ...
    "It runs published portfolio-construction methods over historical price data and "
    "shows you the result. The risk level you pick is a modelling input — it slides "
    "between minimum-variance, equal-risk-contribution and equal weighting — not a "
